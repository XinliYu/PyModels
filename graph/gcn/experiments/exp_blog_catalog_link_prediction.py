...
dblp_folder = path.join(data_folder_path, 'blogcatalog')

adjacency_matrix, graph_nodes = load_graph_adjacency(data_folder=dblp_folder,
                                                     edge_tuple_file='blogcatalog_edges.txt',
                                                     node_transform=int,
...
embedding_files = ("blog_line1_vec.txt", "blog_line2_vec.txt", "blogcatalog_node2vec_vec.txt", "blog_gcn_vector.csv")
...
for embedding_file, skip_first_line, first_element_index in zip(embedding_files, skip_first_line_flags, first_element_index_flags):
    ...
    for row in node_embeddings:
        curr_idx = row[0]
        diff = int(curr_idx - prev_idx - 1)
        if diff > 0:
            for i in range(diff):
                node_embeddings2.append(np.ones(len(node_embeddings[0]), dtype=np.float))
        node_embeddings2.append(row)
        prev_idx = curr_idx

    node_embeddings = np.array(node_embeddings2)
    if first_element_index:
