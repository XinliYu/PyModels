...
aminer_folder = path.join(path.dirname(path.abspath(__file__)), 'aminer')
sys.stdout = flogger(log_file_path(aminer_folder))

weighted_convolution_trials = [False, True]
...
adjacency_matrix, features, labels, onehot_labels, train_mask, validation_mask, test_mask = \
    load_graph_data(data_folder=aminer_folder,
...
if not data_load_only:
    ...
    experiment(exp_name='dblp_gcn',
               ...
               exp_saves_folder=path.join(aminer_folder, 'embeddings'),
